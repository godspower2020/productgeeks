...
export const listSearchedProduct = (value) => async (dispatch) => {
    try {
      dispatch({ type: PRODUCT_SEARCHED_LIST_REQUEST });
  
      const { data } = await API.get(`/api/products/search?keyword=${encodeURIComponent(value)}`);
  
      dispatch({
        type: PRODUCT_SEARCHED_LIST_SUCCESS,
        payload: data,
      });
    } catch (error) {
      dispatch({
        type: PRODUCT_SEARCHED_LIST_FAIL,
        payload:
          error.response && error.response.data.message
            ? error.response.data.message
            : error.message,
